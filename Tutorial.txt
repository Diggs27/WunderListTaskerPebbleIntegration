Tools: Tasker, AutoWeb, Wunderlist, Glance For Pebble, AutoNotification
...
The application is currently in beta, but I have not had any issues with it yet.
This will make the setup much easier. You can also use the Tasker plugin called RESTask for Takser, but I find it has lots of bugs. Autoweb more stable and user friendly.

Here is the link to the play store:

https://play.google.com/store/apps/details?id=com.joaomgcd.autoweb

I hosted my AutoWeb configuration file for AutoWeb. It contains all the setup you need to 
Connect to the Wunderlist webservices. Click the "+" button on the top of the screen and press
IMPORT FROM FILE. Use the config file downloaded above.

*Tasker Setup
Now it's time to setup up Tasker. 

Go out to my git hub and download the tasker task. Import this file into tasker.
...
These are the two peices of data I had you save to notepad in the first setups of this tutorial.
...
This tasks will take care of all the actions. It will
...
Go out and download it on my github:
...
We can do this with AutoNotification and Wunderlist Webhooks.

Once you download AutoNotification, there will be google shortcut link that you can use to configure
AutoNotification for you device. This will bring you into the Messaging service.
...
The I used the Webhook API to send a JSON message which contains the list id and the URL we created in AutoRemote.
...
*Other suggestions
-I also setup at midnight that will run the task as well. This will get will allow our our watch face to stay updated from day to day.
...
-I setup a task to also run every hour to keep the numbers up to date. Sometimes there is network hiccups which cause the task to run or reach certain services. This tasks helps keep things in synch if there are ever any issues.
-Final note, there is another application WunderVoice for Pebble where you can add items directly to your Wunderlist inbox with voice commands.
